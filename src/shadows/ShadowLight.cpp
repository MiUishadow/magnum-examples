...
void ShadowLight::setupShadowmaps(Int numShadowLevels, const Vector2i& size) {
    ...
    for (std::int_fast64_t i = 0; i < numShadowLevels; ++i) {
...
ShadowLight::ShadowLayerData::ShadowLayerData(const Vector2i& size): shadowFramebuffer{{{0, 0}, size}} {}

void ShadowLight::setTarget(const Vector3& lightDirection, const Vector3& screenDirection, SceneGraph::Camera3D& mainCamera) {
    const Matrix4 cameraMatrix = Matrix4::lookAt({0,0,0}, -lightDirection, screenDirection);
    ...
    for(std::size_t layerIndex = 0; layerIndex != _layers.size(); ++layerIndex) {
        std::vector<Vector3> mainCameraFrustumCorners = cameraFrustumCorners(mainCamera, Int(layerIndex));
...
std::vector<Vector3> ShadowLight::getLayerFrustumCorners(SceneGraph::Camera3D& mainCamera, const Int layer) {
...
void ShadowLight::render(SceneGraph::DrawableGroup3D& drawables) {
    ...
    for(std::size_t layer = 0; layer != _layers.size(); ++layer) {
        ...
        for(std::size_t drawableIndex = 0; drawableIndex < drawables.size(); drawableIndex++) {
            ...
            const Vector4 localCentre{0, 0, 0, 1};
